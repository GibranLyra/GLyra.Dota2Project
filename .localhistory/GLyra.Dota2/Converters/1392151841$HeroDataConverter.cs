...
namespace Dota.CentralDota.Repositories
{
    class HeroDataConverter
    {
        ...
        Dictionary<string, Dictionary<string, string>> GetSkillRemainingValues(HtmlDocument doc)
        {
            ...
            var abilityHeaderBox = doc.DocumentNode.SelectNodes("//*[@class = 'abilityHeaderBox']");
            ...
            foreach (var remainingValue in abilityFooterBoxRight)
            {
                ...
                if (divInnerHtml != null)
                {
                    ...
                    if (descriptionList.Count <= 0)
                    {
                        ...
                        foreach (var item in abilityHeaderBox[0].ChildNodes[3].ChildNodes)
	                    {
	                        Debug.WriteLine(item.Name);
	                    }
                        skillNamesList.Add(abilityHeaderBox[0].ChildNodes[3].ChildNodes.Where(x => x.Name == "h2").First().InnerText.Trim());
