...
namespace GLyra.Dota2.Converters
{
    public class SkillCreator
    {
        ...
        {            
			...
			{
			    abilityTypeList.Add(new AbilityType(){Name = abilityTypeArray[i]});
			}

             foreach (var abilitType in abilityTypeList)
             {
        ...
        Skill insertSkill()
        {
            ...
            using (Dota2Entities ctx = new Dota2Entities())
            {
                try
                {
	                ...
	                {
		                skillAbilityTypesCreator.insert(this.skill.ID, abilityType.ID, ctx);
	                }
