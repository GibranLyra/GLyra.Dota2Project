        ...
        List<List<string>> GetSkillRemainingValues(HtmlDocument doc)
        {
            ...
            foreach (var remainingValue in abilityFooterBoxRight)
            {
                ...
                for (int i = 0; i < brList.Count; i++)
                {
                    ...
                    if(i == 0)
                        skillDescriptionList.Add(brList[i].PreviousSibling.PreviousSibling.InnerText.Trim());

                    //When the value is "" the skill must be a scepter upgrade, so we need to check the scepterVal class
                    if (brList[i].NextSibling.InnerText.Trim() == "")
                    {
                        brList.Add(brList[i].ParentNode.SelectNodes(".//*[@class = 'scepterVal']").First());
                        divInnerHtml.Remove(brList[i].ParentNode.SelectNodes(".//*[@class = 'scepterVal']").First());
                    }
                    else
                        skillDescriptionList.Add(brList[i].NextSibling.InnerText.Trim());
                }

                foreach (var span in spanList)
                {
                    spanTextList.Add(span.InnerText.Trim());
                }
